...
- [ ] 1.2 Re-read the spec FSH instance `ViewDefinitionRun` in
...
- [ ] 1.4 Skim the sibling JSON files in
...
- [ ] 2.1 Run `git mv sof-js/metadata/OperationDefinition/\$run.json sof-js/metadata/OperationDefinition/\$viewdefinition-run.json`
      so the filename matches the spec operation code and history is
      preserved.
- [ ] 2.2 In the renamed file, set identification fields:
      ...
      `design.md`).
- [ ] 2.3 Carry across or rewrite the `description` to mirror the FSH
      `Description` field ("Execute a view definition against supplied or
      server data.").
- [ ] 2.4 Rename the existing `format` input parameter to `_format`, set
...
- [ ] 2.8 Update `patient`: cap `max` at `"1"`, scope
      `["system","type","instance"]`, type `Reference`, carry the FSH
      documentation.
- [ ] 2.9 Update `group`: keep `max = "*"`, scope
      `["system","type","instance"]`, type `Reference`, carry the FSH
      documentation.
- [ ] 2.10 Replace the two same-named `source` parameters with two
...
- [ ] 2.11 Add `_limit` (type `integer`, `0..1`, scope
      `["system","type","instance"]`) with the FSH documentation
      ("Maximum number of rows to return.").
- [ ] 2.12 Update `_since`: scope `["system","type","instance"]`, type
      `instant`, `0..1`, carry the FSH documentation.
- [ ] 2.13 Remove the `_count` and `_page` input parameters; the spec does
      not include a paging model for `$viewdefinition-run`.
- [ ] 2.14 Update the output `return` parameter: type `Binary`, `1..1`,
      with documentation matching the FSH ("Transformed data encoded in
      the requested output format.").

## 3. Keep the reference server booting

- [ ] 3.1 In `sof-js/src/server/run.js`, update the `read(req.config,
...
- [ ] 3.2 Grep the rest of `sof-js/` for any other reference to the
      `OperationDefinition` id `$run` (e.g., links, tests, fixtures); fix
      any that would break the boot or the form page.

## 4. Verification

- [ ] 4.1 Run `bun -e "JSON.parse(require('fs').readFileSync('sof-js/metadata/OperationDefinition/\$viewdefinition-run.json','utf-8'))"`
      (or equivalent) to confirm the file parses as JSON.
- [ ] 4.2 From the repo root, build the IG (`npm run build:ig` or
...
- [ ] 4.3 In `sof-js`, run `bun install` then `bun test`; confirm no test
      regressions caused by the metadata change.
- [ ] 4.4 Start the reference server locally (`cd sof-js && bun run start`
...
- [ ] 4.5 Verify the file passes `npm run validate` (the AJV check used
      for the JSON test suite); if `validate` does not cover
      `sof-js/metadata/`, document that explicitly in the PR description.
- [ ] 4.6 Run `openspec validate align-viewdefinition-run-reference --strict`
      and resolve any reported issues.

## 5. Pull request

- [ ] 5.1 Stage only the change directory, the renamed JSON file, and the
